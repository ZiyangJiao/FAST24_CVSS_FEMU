...
static void ssd_init_lines(struct ssd *ssd)
{
    ...
    /* precondition with 61GiB cold data*/
    /*
    for (int i = 0; i < 107; i++) {
...
static struct line *get_next_free_line(struct ssd *ssd)
{
    ...
    // if (((ssd->sp).wl == 1) && lm->free_line_cnt < 80) {
    //     struct nand_block *block_tmp = NULL;
    //     int hottest = 0;
    //     struct ppa ppa;
    //     ppa.g.ch = 0;
...
//    spp->pgs_per_blk = 256*4*2; /* BLK = 8MiB */
    spp->pgs_per_blk = 256*4/2; /* BLK = 4MiB */
    spp->blks_per_pl = 256*2*2; /* 128GiB */
    spp->pls_per_lun = 1;
    spp->luns_per_ch = 8;
    spp->nchs = 8;
...
//    spp->gc_thres_pcent = 0.85;
    ...
    spp->epsilon = 0.00148;
//    spp->alpha = 0.000000000390;
    spp->alpha = 0.000000516375983;
    spp->k = 2.05;
//    spp->gamma = 0.00000000373;
...
//            double ec = (double)blk->erase_cnt / spp->endurance * 10000 ;
...
//                if (spp->wl == 0 && blk->erase_cnt <= 208){
                if (spp->wl == 0 && blk->erase_cnt <= 185){
                    read_retry = 2;
                }
//                if (spp->wl == 0 && blk->erase_cnt <= 144){
...
{
//    if ((ssd->sp).wl == 0) {
//        (ssd->sp).acceleration = 2;
//    } else if ((ssd->sp).acceleration == 1) {
//        (ssd->sp).acceleration = 10;
//    }
    struct line_mgmt *lm = &ssd->lm;
...
//    int ec = -1;
    ...
    if (victim_line == NULL) {
        ...
        if (false) { // check d[1] has biggest ipc
            struct line *line = NULL;
            int min;
            min = victim_line->ipc;
            for (int i = 0; i < lm->victim_line_cnt; i++) {
                ftl_assert(i < pqueue_size(lm->victim_line_pq));
                line = lm->victim_line_pq->d[i+1];
                if (line->ipc > min) {
                    ftl_log("victim_line_pq error!\n");
                    break;
                }
            }
        }
        ...
        // set preference (lower id first) in victim line selection process
        struct line *line = NULL;
        struct nand_block *block_tmp = NULL;
        struct ppa ppa;
        ppa.g.ch = 0;
        ...
        if (((ssd->sp).wl == 0) && (ssd->cv_moderate == 0)) {
            for (int i = 1; i < lm->victim_line_cnt; i++) {
                ...
                } else if (line->vpc == victim_line->vpc && (block_tmp->erase_cnt > block_victim->erase_cnt)) {
                    victim_line = line;
                    block_victim = block_tmp;
                } else if (line->vpc == victim_line->vpc && (block_tmp->erase_cnt == block_victim->erase_cnt) && (line->id < victim_line->id)){
                    victim_line = line;
                    block_victim = block_tmp;
                }
            }
...
//    pqueue_pop(lm->victim_line_pq);
    // if (ssd->cv_moderate == 0 && victim_line->vpc > 0){
    if (ssd->cv_moderate == 0 && victim_line->vpc > ssd->sp.pgs_per_line / 4){
        ssd->cv_moderate = 1;
    }
...
//    ftl_log("GC victim_line %d (%d/%d), valid pages = %d, invalid pages = %d!\n", victim_line->id, ec, (ssd->sp).endurance, victim_line->vpc, victim_line->ipc);
    // ftl_log("GC victim_line %d, valid pages = %d, invalid pages = %d!\n", victim_line->id, victim_line->vpc, victim_line->ipc);
    return victim_line;
}
...
static int do_gc(struct ssd *ssd, bool force)
{
    ...
    struct line_mgmt *lm = &ssd->lm;
    struct line *line;
//    double ec = (double)block->erase_cnt;
//    double rc = (double)0;
//    double rber = spp->epsilon + spp->alpha*pow(ec,spp->k) + spp->gamma*pow(ec,spp->p)*pow(rc,spp->q);
//    int bits_count = spp->secs_per_pg * spp->secsz * 8;
//    int bits_err = (int)bits_count * rber;
//    if (block->erase_cnt >= spp->endurance || (spp->wl == 0 && block->erase_cnt >= 298)) {
//    if (block->erase_cnt >= spp->endurance || (spp->wl == 0 && block->erase_cnt >= 208)) {
    if ((spp->wl == 0) && (ssd->cv_moderate) && (block->erase_cnt >= 270) && (block->erase_cnt < 280)) {
        goto maintainCapacity;
    }
    // if (block->erase_cnt >= spp->endurance || (spp->wl == 0 && (ssd->cv_moderate == 0) && (block->erase_cnt >= 280)) || (spp->wl == 0 && (ssd->cv_moderate == 1) && (block->erase_cnt >= 290))) {
    if (block->erase_cnt >= spp->endurance || (spp->wl == 0 && (block->erase_cnt >= 280))) {
        line = get_line(ssd, &ppa);
        line->ipc = 0;
        line->vpc = 0;
...
//                int threshold = erase_sum / spp->tt_lines * 3 / 4 + spp->endurance / 4;
            ...
            //    int threshold = erase_sum / wl_lines + 20;
...
static uint64_t ssd_write(struct ssd *ssd, NvmeRequest *req)
{
    ...
    while (should_gc_high(ssd)) {
        ...
        if (r == -1)
            break;
//        while (ssd->lm.free_line_cnt <= 26 && pqueue_peek(ssd->lm.victim_line_pq)->vpc == 0) {
//            r = do_gc(ssd, true);
//        }
    }
