...
static uint64_t ssd_advance_status(struct ssd *ssd, struct ppa *ppa, struct nand_cmd *ncmd)
{
    ...
    // read-heavy data relocation
    int avg_ec = (int)(spp->total_ec/spp->tt_blks);
    // check if target is an aged (super)block
    if (c == NAND_READ && spp->wl != 1 && ssd->cv_moderate == 0 && blk->erase_cnt > avg_ec) {
        struct line_mgmt *lm = &ssd->lm;
        struct ppa ppa_tmp;
        ...
        for (i = 0; i < lm->tt_lines; i++) { // calculate avg_read and find a candidate to swap
            ppa_tmp.g.blk = i;
            ppa_tmp.g.pl = 0;
            line_read = 0;
            ...
            if ((i == (&ssd->wp)->curline->id) || (lm->lines[i].vpc == 0)) {
                continue;
            }
            if (blk_tmp->erase_cnt < avg_ec && line_read < line_avg_read*0.8) { // blk->erase_cnt < blk_tmp->erase_cnt due to blk->erase_cnt > avg_ec
    ...
    switch (c) {
        case NAND_READ:
            ...
            if (read_retry != 0 && spp->wl == 0) {
                lun->next_lun_avail_time = nand_stime + spp->pg_rd_lat * (1 + read_retry) -30000;
            } else {
...
static struct line *select_victim_line(struct ssd *ssd, bool force)
{
    ...
    if (victim_line == NULL) { // normal case
        ...
        if ((ssd->sp).wl == 0 && (ssd->cv_moderate == 0)) {
            ...
            line_score = victim_line->ipc/spp->pgs_per_line*0.5 + 2*block_victim->erase_cnt/spp->endurance;
            for (int i = 1; i < lm->victim_line_cnt; i++) {
                line = lm->victim_line_pq->d[i + 1]; // d[1] is the result of peek(), we check the rest.
                ppa.g.blk = line->id;
                block_tmp = get_blk(ssd, &ppa);
                total_vpc += line->vpc;
                line_score_tmp = line->ipc/spp->pgs_per_line*0.5 + 2*block_tmp->erase_cnt/spp->endurance;
