...
{
    /* Ziyang:
     * output endurance log when smart info is required
     * The "Struct SSD" is defined and inited in 'ftl.h'
     */
    printf("Ziyang : %s\n", n->ssd->ssdname);
//    TEST: FEMU use host directory: Current working dir: /home/zjiao04/femu/build-femu
//    char cwd[100];
...
//        return 1;
//    }
    FILE *fp= fopen("/home/zjiao04/femu/build-femu/endurance_log.txt", "a+");
    if (fp != NULL) {
...
//                    printf("%d ", b->erase_cnt);
                        fprintf(fp, "%d ", b->erase_cnt);
//                        b->erase_cnt = b->erase_cnt * 2;
                    }
                }
            }
        }
        ...
        if (false) { // prediction and state transition
            int ret = system("python3 FF.py");
            if (ret != -1) {
                fp= fopen("/home/zjiao04/femu/build-femu/endurance_log.txt", "r");
                if (fp == NULL) {
                    printf("Update EC: Endurance log file open error!\n");
                } else {

                    char * line = malloc(sizeof(char) *chs*luns*pls*blks*10+1);
...
//            printf("Update EC: [cnt = %d]\n", cnt);
//            printf("%d\n", record[chs*luns*pls*blks]); // atoi("\n") = 0
                    cnt = 0;
                    for (i = 0; i < chs; i++) {
                        for (j = 0; j < luns; j++) {
                            for (k = 0; k < pls; k++) {
                                for (m = 0; m < blks; m++) {
                                    ...
                                    struct nand_plane *p = &(l->pl[k]);
                                    struct nand_block *b = &(p->blk[m]);
                                    b->erase_cnt = record[cnt];
                                    cnt++;
                                }
                            }
                        }
                    }
                    printf("Update EC: Completed! [cnt = %d]\n", cnt);
                    free(line);
                    fclose(fp);
                }
            }
